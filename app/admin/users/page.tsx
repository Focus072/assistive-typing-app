...
export default function AdminUsersPage() {
  ...
  const handleBulkTierChange = async () => {
    ...
    try {
      const res = await fetch("/api/admin/bulk/change-tier", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userIds: Array.from(selected), planTier: bulkTier }),
      })
      if (res.ok) {
        setSelected(new Set())
        fetchUsers()
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
        ...
        <div className="max-w-7xl mx-auto pl-14 md:pl-4 pr-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              {acceptedOnly && (
                <Link href="/admin/users" className="hidden sm:flex items-center gap-2 min-h-[44px] px-4 py-3 text-zinc-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors text-sm font-medium">
                  All users
                </Link>
              )}
              <Link
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        {/* Bulk Actions */}
        {selected.size > 0 && (
          <div className="mb-4 flex flex-wrap items-center gap-3 px-4 py-3 rounded-lg bg-violet-500/10 border border-violet-500/20">
            <span className="text-sm text-violet-300">{selected.size} selected</span>
            <select
              value={bulkTier}
              onChange={(e) => setBulkTier(e.target.value)}
              className="text-xs bg-zinc-900 border border-white/10 rounded px-2 py-1.5 text-white"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:bg-zinc-700 rounded-lg text-sm font-medium transition-colors"
            >
              {bulkChanging ? <Loader2 className="w-3 h-3 animate-spin" /> : <Users className="w-3 h-3" />}
              Change Tier
            </button>
            <button onClick={() => setSelected(new Set())} className="text-xs text-zinc-400 hover:text-white ml-auto">
              Clear
            </button>
          </div>
        )}
        ...
        <div className="md:hidden space-y-4">
          {users.length === 0 ? (
            <div className="rounded-xl bg-white/5 border border-white/10 p-6 text-center text-zinc-500">No users found</div>
          ) : (
            users.map((user) => (
              <div key={user.id} className="rounded-xl bg-white/5 border border-white/10 p-4">
                <div className="flex items-start gap-4">
                  <input type="checkbox" checked={selected.has(user.id)} onChange={() => toggleSelect(user.id)} className="mt-1 rounded border-white/10" />
                  {user.image ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3">
                      ...
                      <SetTierButton userId={user.id} currentTier={user.planTier ?? "FREE"} onUpdated={() => fetchUsers()} />
                    </div>
                    <Link href={`/admin/jobs?userId=${user.id}`} className="mt-3 inline-block text-sm text-violet-400 hover:text-violet-300">
                      View Jobs →
                    </Link>
                  </div>
                </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="px-4 py-3 text-left">
                    <input type="checkbox" checked={selected.size === users.length && users.length > 0} onChange={toggleAll} className="rounded border-white/10" />
                  </th>
                  ...
                  {acceptedOnly && <th className="px-6 py-3 text-left text-xs font-medium text-zinc-400 uppercase">Accepted</th>}
              ...
              <tbody>
                {users.length === 0 ? (
                  <tr>
                    <td colSpan={acceptedOnly ? 9 : 8} className="px-6 py-8 text-center text-zinc-500">No users found</td>
                  </tr>
                ) : (
                  users.map((user) => (
                    <tr key={user.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td className="px-4 py-4">
                        <input type="checkbox" checked={selected.has(user.id)} onChange={() => toggleSelect(user.id)} className="rounded border-white/10" />
                      </td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        {user.subscriptionStatus && <p className="text-xs text-zinc-500">{user.subscriptionStatus}</p>}
                        <SetTierButton userId={user.id} currentTier={user.planTier ?? "FREE"} onUpdated={() => fetchUsers()} />
                      </td>
                      ...
                      {acceptedOnly && (
                        <td className="px-6 py-4 text-sm text-emerald-400">
                          {user.academicIntegrityAcceptedAt ? new Date(user.academicIntegrityAcceptedAt).toLocaleDateString() : "—"}
                        </td>
                      )}
                      <td className="px-6 py-4">
                        <Link href={`/admin/jobs?userId=${user.id}`} className="text-violet-400 hover:text-violet-300 text-sm font-medium">
        ...
        {pagination.totalPages > 1 && (
          <div className="md:hidden mt-6 flex items-center justify-between">
            <button
              onClick={() => setPagination(p => ({ ...p, page: p.page - 1 }))}
              disabled={pagination.page === 1}
              className="min-h-[44px] px-4 py-3 rounded-lg bg-white/10 border border-white/10 text-zinc-300 disabled:opacity-50 text-sm font-medium"
            >
              Previous
            </button>
