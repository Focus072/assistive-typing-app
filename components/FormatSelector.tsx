"use client";

import { useState } from "react";
import type { DocumentFormat } from "@/types";
import { formatConfigs } from "@/lib/document-formats";
import { FormatMetadataModal, type FormatMetadata } from "./FormatMetadataModal";
import { CustomFormatModal, type CustomFormatConfig } from "./CustomFormatModal";
import { useDashboardTheme } from "@/app/dashboard/layout";

interface FormatSelectorProps {
  value: DocumentFormat;
  onChange: (format: DocumentFormat, metadata?: FormatMetadata, customConfig?: CustomFormatConfig) => void;
  formatMetadata?: FormatMetadata;
  customFormatConfig?: CustomFormatConfig;
}

export function FormatSelector({
  value,
  onChange,
  formatMetadata,
  customFormatConfig,
}: FormatSelectorProps) {
  const formats: DocumentFormat[] = ["none", "mla", "custom"];
  const [showMetadataModal, setShowMetadataModal] = useState(false);
  const [showCustomModal, setShowCustomModal] = useState(false);
  const [pendingFormat, setPendingFormat] = useState<DocumentFormat | null>(null);
  const { isDark } = useDashboardTheme();

  const handleFormatClick = (format: DocumentFormat) => {
    if (format === "none") {
      onChange(format);
      return;
    }

    if (format === "custom") {
      setPendingFormat(format);
      setShowCustomModal(true);
      return;
    }

    // Always show modal for formats that require metadata
    // This lets users configure or edit metadata any time they click the format.
    setPendingFormat(format);
    setShowMetadataModal(true);
  };

  const handleMetadataSave = (metadata: FormatMetadata) => {
    if (pendingFormat) {
      onChange(pendingFormat, metadata);
      setPendingFormat(null);
    }
  };

  const handleCustomConfigSave = (config: CustomFormatConfig) => {
    if (pendingFormat === "custom") {
      onChange("custom", undefined, config);
      setPendingFormat(null);
    }
  };

  return (
    <div className="space-y-3">
      <label
        className={`text-sm font-medium flex items-center gap-2 ${
          isDark ? "text-white" : "text-black"
        }`}
      >
        <svg
          className={`w-4 h-4 ${isDark ? "text-white" : "text-black"}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
          />
        </svg>
        How it looks
      </label>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
        {formats.map((format) => {
          const config = formatConfigs[format];
          const isSelected = value === format;

          return (
            <button
              key={format}
              type="button"
              onClick={() => handleFormatClick(format)}
              className={`relative p-3 md:p-4 rounded-lg text-left transition-all group touch-manipulation active:scale-95 ${
                isSelected
                  ? isDark
                    ? "bg-white border-white border-2 text-black"
                    : "bg-black text-white border border-black"
                  : isDark
                  ? "bg-black/40 border border-white/20 hover:bg-black/60"
                  : "bg-white border border-black hover:bg-gray-50 active:bg-gray-100"
              }`}
              aria-label={`Select ${config.name} format`}
              aria-pressed={isSelected}
            >
              <div
                className={`w-7 h-7 md:w-8 md:h-8 rounded-lg mb-2 flex items-center justify-center ${
                  isSelected
                    ? isDark
                      ? "bg-black text-white"
                      : "bg-white text-black"
                    : isDark
                    ? "bg-white/10 text-white group-hover:text-white"
                    : "bg-black/5 text-black group-hover:text-black"
                }`}
              >
                <svg
                  className="w-3.5 h-3.5 md:w-4 md:h-4"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
                  />
                </svg>
              </div>
              <div
                className={`font-medium text-xs md:text-sm ${
                  isSelected
                    ? isDark
                      ? "text-black"
                      : "text-white"
                    : isDark
                    ? "text-white"
                    : "text-black"
                }`}
              >
                {config.name}
              </div>
              <div
                className={`text-xs hidden sm:block mt-0.5 ${
                  isSelected
                    ? isDark
                      ? "text-black/80"
                      : "text-white/80"
                    : isDark
                    ? "text-white/60"
                    : "text-black/60"
                }`}
              >
                {config.description}
              </div>

              {isSelected && (
                <div className="absolute top-2 right-2">
                  <div className="w-2 h-2 rounded-full bg-white" />
                </div>
              )}
            </button>
          );
        })}
      </div>

      {value !== "none" && (
        <div
          className={`px-3 py-2 rounded-lg border ${
            isDark ? "bg-white/5 border-white/15" : "bg-black/5 border-black/20"
          }`}
        >
          <div className="flex items-center justify-between mb-1">
            <p className={`text-xs ${isDark ? "text-white" : "text-black"}`}>
              <strong>{formatConfigs[value].name}:</strong>{" "}
              {formatConfigs[value].description}
            </p>
            {(formatMetadata || (value === "custom" && customFormatConfig)) && (
              <span
                className={`text-xs px-2 py-0.5 rounded border ${
                  isDark
                    ? "bg-green-500/10 text-green-200 border-green-500/40"
                    : "bg-green-50 text-green-800 border-green-300"
                }`}
              >
                ✓ Configured
              </span>
            )}
          </div>
          <div
            className={`mt-1 text-xs space-y-0.5 ${
              isDark ? "text-white/70" : "text-black/70"
            }`}
          >
            {value === "custom" && customFormatConfig ? (
              <>
                <p>
                  • Font: {customFormatConfig.fontFamily},{" "}
                  {customFormatConfig.fontSize}pt
                </p>
                <p>
                  • Line spacing:{" "}
                  {customFormatConfig.lineSpacing === 2.0
                    ? "Double"
                    : customFormatConfig.lineSpacing === 1.5
                    ? "1.5"
                    : customFormatConfig.lineSpacing === 1.0
                    ? "Single"
                    : customFormatConfig.lineSpacing}
                </p>
                <p>
                  • Margins: {customFormatConfig.margins.top}&quot; top,{" "}
                  {customFormatConfig.margins.right}&quot; right,{" "}
                  {customFormatConfig.margins.bottom}&quot; bottom,{" "}
                  {customFormatConfig.margins.left}&quot; left
                </p>
                {customFormatConfig.firstLineIndent > 0 && (
                  <p>
                    • First line indent: {customFormatConfig.firstLineIndent}&quot;
                  </p>
                )}
              </>
            ) : (
              <>
                <p>
                  • Font: {formatConfigs[value].fontFamily},{" "}
                  {formatConfigs[value].fontSize}pt
                </p>
                <p>
                  • Line spacing:{" "}
                  {formatConfigs[value].lineSpacing === 2.0
                    ? "Double"
                    : formatConfigs[value].lineSpacing === 1.5
                    ? "1.5"
                    : "Single"}
                </p>
                <p>• Margins: {formatConfigs[value].margins.top}&quot; all around</p>
                {formatConfigs[value].firstLineIndent > 0 && (
                  <p>
                    • First line indent: {formatConfigs[value].firstLineIndent}&quot;
                  </p>
                )}
                {formatMetadata && (
                  <p className="text-green-700 mt-1 pt-1 border-t border-gray-300">
                    • Header: {formatMetadata.studentName},{" "}
                    {formatMetadata.courseName}
                  </p>
                )}
              </>
            )}
          </div>
        </div>
      )}

      <FormatMetadataModal
        isOpen={showMetadataModal}
        format={pendingFormat || value}
        onClose={() => {
          setShowMetadataModal(false);
          setPendingFormat(null);
        }}
        onSave={handleMetadataSave}
        initialMetadata={formatMetadata}
      />
      <CustomFormatModal
        isOpen={showCustomModal}
        onClose={() => {
          setShowCustomModal(false);
          setPendingFormat(null);
        }}
        onSave={handleCustomConfigSave}
        initialConfig={customFormatConfig}
      />
    </div>
  );
}


